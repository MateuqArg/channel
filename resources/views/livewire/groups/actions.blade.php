<a id="send-btn" data-receiver="{{ $visitor->name }}" data-bs-toggle="modal"data-bs-target="#send"><i class="bi bi-envelope btn btn-outline-success"></i></a>
...
{{-- <div class="modal fade" wire:ignore.self id="send" tabindex="-1" aria-labelledby="createLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal"><i class="bi bi-arrow-left"></i></button>
        <button wire:click="sendEmail('{{ $group->title }}')" class="btn btn-success"><i class="bi bi-send"></i> ENVIAR</button>
      </div>
    </div>
  </div>
</div> --}}
